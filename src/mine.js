  ...
  window.Buffer={
    from:function(data,enc){
      if(typeof data==='string'){
        if(enc==='base64'){const bin=atob(data),arr=new Uint8Array(bin.length);for(let i=0;i<bin.length;i++)arr[i]=bin.charCodeAt(i);return arr;}
...
async function saveEnc(secret,pw,kp){const salt=crypto.getRandomValues(new Uint8Array(16)),iv=crypto.getRandomValues(new Uint8Array(12)),key=await pwKey(pw,salt);const ct=await crypto.subtle.encrypt({name:'AES-GCM',iv},key,new TextEncoder().encode(secret));localStorage.setItem(STORE_KEY,JSON.stringify({salt:Array.from(salt),iv:Array.from(iv),ct:Array.from(new Uint8Array(ct)),hint:kp?kp.publicKey.toString().slice(0,8)+'…':''}));}
...
async function kpFromSecret(secret){
  ...
  if(secret.startsWith('phrase:')){const words=secret.slice(7).trim().replace(/\s+/g,' ').toLowerCase().split(' ');const seed=await mnemonicToSeed(words.join(' '));return sol.Keypair.fromSeed(await slip10(seed,"m/44'/501'/0'/0'"));}
  if(secret.startsWith('privkey:')){const raw=secret.slice(8).trim();let bytes;try{bytes=B58.decode(raw)}catch{bytes=raw.startsWith('[')?new Uint8Array(JSON.parse(raw)):Uint8Array.from(raw.match(/.{1,2}/g).map(b=>parseInt(b,16)))}return bytes.length===64?sol.Keypair.fromSecretKey(bytes):sol.Keypair.fromSeed(bytes);}
...
async function signMessage(message){
  ...
  await _loadNacl();
  const kp=_wallet.keypair,msgBytes=new TextEncoder().encode(message);
  const nacl=window.nacl||window.TweetNaCl;
  if(!nacl||!nacl.sign)throw Error('nacl not available');
...
function lockWallet(){if(_wallet.keypair&&_wallet.keypair.secretKey)_wallet.keypair.secretKey.fill(0);_wallet={locked:true,keypair:null,pubkey:null,connection:null};_pdas={};clearInterval(_cycleTimer);clearInterval(_claimTimer);_updateFigureBtn();}
...
function showProofOfWill(){
  ...
  wrap.innerHTML=
    ...
    '<div style="display:grid;grid-template-columns:1fr;gap:6px;margin-bottom:24px">'+
      _powItem('Anti-spam by construction','You cannot flood the registry with mediocre files. Each new sphere must improve your ratio.','#ff4560')+
...
function _powVar(key,desc){
  return '<div style="background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.05);border-radius:6px;padding:8px 10px;display:flex;gap:8px;align-items:flex-start">'+
    '<span style="font-family:monospace;font-size:13px;color:#f0a830;flex-shrink:0;min-width:14px">'+key+'</span>'+
    '<span style="font-size:11px;color:rgba(240,240,248,.5);line-height:1.5">'+desc+'</span>'+
  '</div>';
}
function _powItem(title,desc,color){
  return '<div style="background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.05);border-left:2px solid '+color+';border-radius:0 8px 8px 0;padding:10px 12px">'+
    '<div style="font-size:12px;font-weight:600;color:'+color+';margin-bottom:3px">'+title+'</div>'+
    '<div style="font-size:11px;color:rgba(240,240,248,.5);line-height:1.5">'+desc+'</div>'+
  '</div>';
}
...
function renderLocked(body){
  ...
  const S=(s)=>'style="'+s+'"';
  const wrap=document.createElement('div');
  wrap.style.cssText='flex:1;display:flex;flex-direction:column;justify-content:center;padding:20px 16px;overflow-y:auto';
  const inner=document.createElement('div');
  inner.style.cssText='width:100%;max-width:360px;margin:0 auto';
  ...
  if(!has){
    inner.innerHTML=
      ...
      '<details '+S('border:1px solid var(--border,rgba(255,255,255,.08));border-radius:var(--r-sm,8px);padding:8px 12px')+'>'+
    ...
    inner.innerHTML=
      ...
      '<details '+S('border:1px solid var(--border,rgba(255,255,255,.08));border-radius:var(--r-sm,8px);padding:8px 12px;margin-bottom:8px')+'>'+
  ...
  if($('mine-create-btn'))$('mine-create-btn').addEventListener('click',async()=>{
    ...
    if(!pw)return showErr(body,'Password required');if(pw!==pw2)return showErr(body,'Passwords do not match');
...
function renderUnlocked(body){
  ...
  const wrap=document.createElement('div');
  wrap.style.cssText='flex:1;display:flex;flex-direction:column;justify-content:center;padding:12px 16px;overflow-y:auto;min-height:0';
  const inner=document.createElement('div');
  ...
  '<div '+S('display:flex;align-items:center;justify-content:space-between;gap:6px;margin-bottom:4px')+'>'+
    ...
    '<div '+S('display:flex;gap:4px')+'>'+
      '<button class="ym-btn ym-btn-ghost" id="mine-pow-btn" '+S('padding:3px 8px;font-size:11px;color:rgba(255,69,96,.7);border-color:rgba(255,69,96,.2)')+'">◈ PoW</button>'+
  ...
  if($('mine-pow-btn'))$('mine-pow-btn').addEventListener('click',showProofOfWill);
