...
async function render(containerArg){
  ...
  addBtn.addEventListener('click',()=>{
    if(_publishOpen){
      _publishOpen=false;
      addBtn.textContent='+';addBtn.style.cssText='flex-shrink:0;width:68px;height:68px;border-radius:50%;background:linear-gradient(135deg,#ffe066,#f0a830,#ff8c00);border:none;color:#06060e;font-size:40px;font-weight:200;cursor:pointer;display:flex;align-items:center;justify-content:center;line-height:1;transition:all .2s;box-shadow:0 0 0 3px rgba(240,168,48,.25),0 0 28px 8px rgba(240,168,48,.7),0 0 60px 16px rgba(240,168,48,.3),0 6px 32px rgba(240,168,48,.5)';
  ...
  function renderTypePills(){
    ...
    [{id:'spheres',label:'⬡ Sphere'},{id:'themes',label:'🎨 Theme'},{id:'photo',label:'📷 Photo'},{id:'video',label:'🎥 Video'}].forEach(opt=>{
      ...
      p.addEventListener('click',()=>{
        ...
        if(_publishOpen){_publishOpen=false;addBtn.textContent='+';addBtn.style.cssText='flex-shrink:0;width:68px;height:68px;border-radius:50%;background:linear-gradient(135deg,#ffe066,#f0a830,#ff8c00);border:none;color:#06060e;font-size:40px;font-weight:200;cursor:pointer;display:flex;align-items:center;justify-content:center;line-height:1;transition:all .2s;box-shadow:0 0 0 3px rgba(240,168,48,.25),0 0 28px 8px rgba(240,168,48,.7),0 0 60px 16px rgba(240,168,48,.3),0 6px 32px rgba(240,168,48,.5)';body.querySelector('#list-type-pills')?.style.setProperty('display','flex');body.querySelector('#list-cat-row')?.style.setProperty('display','flex');['#list-wip-row','#list-search'].forEach(s=>{const el=body.querySelector(s);if(el)el.style.display='';});const sr2=body.querySelector('#list-search-row');if(sr2)sr2.style.display='flex';}
...
function _buildSphereActionBar(sphere, isActive, card, getOpen, setOpen){
  ...
  const bar=_makeActionBar([
    ...
    ...(isActive && !MANDATORY_SPHERES.includes(sphere.fileName) ? [{icon:'◼',label:'Off',style:BTN_DANGER,id:'activate',onClick:async(btn)=>{
      ...
      rebuildBar(card,sphere,nowActive);
    }}] : []),
  ]);
  bar.dataset.barEl='1';
  bar.style.display='none';
