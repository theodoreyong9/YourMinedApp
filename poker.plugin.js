/**
 * FRODON PLUGIN — Texas Hold'em Poker P2P  v4.0
 * Calqué exactement sur TicTacToe :
 *  - tables{} comme games{}
 *  - table.done = true quand terminé (pas de phase 'ended')
 *  - addScore() appelé directement dans onDM showdown / dans hostEnd
 *  - renderTable() affiche résultat si done, sinon jeu en cours
 */
frodon.register({
  id: 'poker', name: "Texas Hold'em", version: '4.0.0',
  author: 'frodon-community',
  description: "Poker Texas Hold'em P2P.",
  icon: '🃏',
}, () => {

  const PLUGIN_ID = 'poker';
  const store = frodon.storage(PLUGIN_ID);
  const tables = {};  // tid -> table (comme games{} dans TicTacToe)
  ...
  function persist() {
    ...
    for(const [tid,t] of Object.entries(tables)) {
      save[tid] = { ...t,
        deck:     t.isHost ? (t.deck||[])     : [],
        allHands: t.isHost ? (t.allHands||{}) : {},
      };
    }
  ...
  function restore() {
    const s = store.get('tables');
    if(!s) return;
    for(const [tid,t] of Object.entries(s)) tables[tid] = { ...t, allHands: t.allHands||{} };
    if(!Object.keys(tables).length) return;
    setTimeout(() => {
      for(const [tid,t] of Object.entries(tables)) {
        if(t.isHost) hostSync(tid);
        else send(t.hostId, 'resync', { tid });
      }
    }, 3500);
  ...
  /* ── Réseau ── */
  // TOUJOURS passer tid dans le payload (appris de TicTacToe : gameId dans chaque DM)
  function send(pid, type, payload) {
    frodon.sendDM(pid, PLUGIN_ID, { type, ...payload, _silent: true });
  }
  ...
  function hostSync(tid) {
    ...
    t.players.forEach(pl => {
      ...
      if(!t.done && t.phase !== 'lobby' && t.allHands[pl.id])
        send(pl.id, 'hand', { tid, cards: t.allHands[pl.id] });
    });
    persist();
    frodon.refreshSphereTab(PLUGIN_ID);
  }
  ...
  function hostDeal(tid) {
    ...
    if(t.players.filter(p => p.chips > 0).length < 2) {
      frodon.showToast('Pas assez de joueurs'); return;
    }
    t.deck = mkDeck(); t.community = []; t.pot = 0;
    t.allHands = {}; t.done = false; t.result = null; t.phase = 'preflop';
    ...
    const sb = t.players[si], bb = t.players[bi];
    const sa = Math.min(t.sb, sb.chips), ba = Math.min(t.bb, bb.chips);
    sb.chips-=sa; sb.bet=sa; if(!sb.chips) sb.status='allin';
    bb.chips-=ba; bb.bet=ba; if(!bb.chips) bb.status='allin';
    t.roundBet = ba;
  ...
  function hostAct(tid, fromId, action, amount) {
    ...
    if(action==='fold')   { p.status='fold'; p.hasActed=true; }
      ...
      const r=Math.max(amount||0, t.roundBet+t.bb), a=Math.min(r-p.bet, p.chips);
      p.chips-=a; p.bet+=a; t.roundBet=p.bet; if(!p.chips) p.status='allin';
      t.players.forEach(o => { if(o.id!==fromId && o.status==='active') o.hasActed=false; });
      p.hasActed=true;
    }
    else if(action==='allin')  {
      const a=p.chips; p.chips=0; p.bet+=a;
      if(p.bet > t.roundBet) { t.roundBet=p.bet; t.players.forEach(o => { if(o.id!==fromId && o.status==='active') o.hasActed=false; }); }
  ...
  function hostRoundEnd(tid) {
    ...
    if(alive.length <= 1) {
      ...
      if(w) {
        ...
        const pot = t.pot; w.chips += pot;
        hostEnd(tid, { pot, winner:w.id, winnerName:w.name,
          results: [{ id:w.id, name:w.name, hand:[], handName:'Tous se sont couchés' }],
  ...
  function hostShowdown(tid) {
    ...
    t.players.forEach(p => { t.pot+=p.bet; p.bet=0; });
    const alive = t.players.filter(p => p.status==='active'||p.status==='allin');
    const ev = alive.map(p => { const h=t.allHands[p.id]||[]; const b=evalBest([...h,...t.community]); return {id:p.id,name:p.name,hand:h,handName:b.name,score:b.score}; });
    ev.sort((a,b) => b.score-a.score);
    const w = ev[0]; const wp = t.players.find(p => p.id===w.id);
    const pot = t.pot; if(wp) wp.chips += pot;
    hostEnd(tid, { pot, winner:w.id, winnerName:w.name, results:ev,
      players:t.players.map(p=>({id:p.id,chips:p.chips})), community:t.community });
  }

  /* ── Fin de main — UNE SEULE FOIS côté hôte ── */
  function hostEnd(tid, result) {
    const t = tables[tid]; if(!t || t.done) return;  // guard double-appel
    ...
    // Score hôte (comme TicTacToe : addScore ici directement)
    const isWin = result.winner === me();
    addScore(isWin?'win':'loss', t.players.filter(p=>p.id!==me()).map(p=>p.name), result.pot);
    // Envoyer aux clients
    sendAll(tid, 'showdown', { result });
    persist();
    frodon.refreshSphereTab(PLUGIN_ID);
  ...
  frodon.onDM(PLUGIN_ID, (fromId, payload) => {
    ...
    if(type === 'invite_ok') {
      ...
      if(!p) { p={id:fromId,name:peer?.name||'?',chips:1000,bet:0,hasActed:false,status:'active'}; t.players.push(p); }
    ...
    if(type === 'sync') {
      ...
      t.currentIdx = p.currentIdx;
      t.dealerIdx  = p.dealerIdx;
      t.roundBet   = p.roundBet;
      t.players    = p.players;
      t.done       = p.done||false;
      t.result     = p.result||null;
      // Réinitialiser myHand si nouvelle main
    ...
    if(type === 'action') {
      const t = tables[tid]; if(!t||!t.isHost) return;
      hostAct(tid, fromId, payload.action, payload.amount||0); return;
    }

    // Résultat reçu par les CLIENTS (hôte ne passe jamais ici)
    if(type === 'showdown') {
      const t = tables[tid]; if(!t||t.isHost||t.done) return;
      const result = payload.result;
      t.done = true; t.result = result;
      result.players.forEach(r => { const p=t.players.find(q=>q.id===r.id); if(p) p.chips=r.chips; });
      if(result.community) t.community = result.community;
      // Score client — directement ici, comme TicTacToe addScore dans onDM
      const isWin = result.winner === me();
      addScore(isWin?'win':'loss', t.players.filter(p=>p.id!==me()).map(p=>p.name), result.pot);
      persist();
      frodon.refreshSphereTab(PLUGIN_ID);
    ...
    if(type === 'kick') {
      ...
      delete tables[tid]; persist();
      frodon.showToast('🃏 Table fermée.'); frodon.refreshSphereTab(PLUGIN_ID); return;
    }
    ...
    if(type === 'leave') {
      ...
      p.status='away';
      if(!t.done && t.phase!=='lobby') {
        if(t.currentIdx === t.players.indexOf(p)) hostAct(tid, fromId, 'fold', 0);
        else hostSync(tid);
      }
      return;
    }
    ...
    if(type === 'resync') {
      ...
      if(!p) {
        ...
        if(p) { if(t.allHands[p.id]) { t.allHands[fromId]=t.allHands[p.id]; delete t.allHands[p.id]; } p.id=fromId; }
      }
  ...
  frodon.onPeerAppear(peer => {
    for(const [tid,t] of Object.entries(tables)) {
      let p = t.players.find(pl => pl.id===peer.peerId) || t.players.find(pl => pl.status==='away' && pl.name===peer.name);
      if(!p) continue;
  ...
  frodon.onPeerLeave(peerId => {
    for(const [tid,t] of Object.entries(tables)) {
      ...
      p.status = 'away';
      if(!t.done && t.phase!=='lobby') {
        if(t.players[t.currentIdx]?.id===peerId) hostAct(tid, peerId, 'fold', 0);
        else hostSync(tid);
      }
    }
  });
  ...
  frodon.registerPeerAction(PLUGIN_ID, '🃏 Poker', (peerId, container) => {
    const peer = frodon.getPeer(peerId), pName = peer?.name||'?';
    // Chercher une table existante où ce pair est impliqué
    const t = Object.values(tables).find(g => g.players.some(p => p.id===peerId));

    if(!t) {
      const btn = frodon.makeElement('button','plugin-action-btn acc','🃏 Inviter à jouer');
      btn.addEventListener('click', () => {
        const my = frodon.getMyProfile(), tid = 'pk_'+Date.now();
        tables[tid] = {
          tid, isHost:true, hostId:me(), phase:'lobby', done:false,
          players: [
            { id:me(), name:my.name, chips:1000, bet:0, hasActed:false, status:'active' },
            { id:peerId, name:pName, chips:1000, bet:0, hasActed:false, status:'active' },
          ],
          myHand:[], allHands:{}, community:[], deck:[], pot:0, roundBet:20,
          currentIdx:0, dealerIdx:-1, sb:10, bb:20, result:null,
        };
        send(peerId, 'invite', { tid, sb:10, bb:20,
          players: tables[tid].players.map(p=>({id:p.id,name:p.name,chips:p.chips,status:p.status})),
          _label: '🃏 Invitation Poker !' });
        persist();
        frodon.showToast('🃏 Invitation envoyée à '+pName);
        frodon.refreshSphereTab(PLUGIN_ID);
        setTimeout(() => frodon.focusPlugin(PLUGIN_ID), 200);
      });
    ...
    if(t.done && t.result) {
      const isWin = t.result.winner === me();
      const st = frodon.makeElement('div','');
      st.style.cssText = 'text-align:center;padding:6px 0;font-size:.72rem;font-family:var(--mono);color:'+(isWin?'var(--ok)':'var(--warn)');
      st.textContent = isWin ? '🏆 Victoire !' : '😔 Défaite';
      container.appendChild(st);
      const go = frodon.makeElement('button','plugin-action-btn acc','▶ Voir résultat');
      go.addEventListener('click', () => frodon.focusPlugin(PLUGIN_ID));
      container.appendChild(go);
    } else {
      const isMine = t.players[t.currentIdx]?.id === me();
      const st = frodon.makeElement('div','');
      st.style.cssText = 'text-align:center;padding:4px 0 6px;font-size:.68rem;color:var(--txt2)';
      st.textContent = t.phase==='lobby' ? '⌛ Salon d\'attente' : isMine ? '⌛ Votre tour' : '💬 Tour de '+(t.players[t.currentIdx]?.name||'?');
      container.appendChild(st);
  ...
  frodon.registerBottomPanel(PLUGIN_ID, [
    { id:'table', label:'🃏 Table',
      render(container) {
        injectCSS();
        // Trouver ma table — cherche d'abord isHost, sinon la seule table non-hôte
        const t = Object.values(tables).find(g => g.isHost) || Object.values(tables)[0] || null;
        if(!t)              { renderEmpty(container); return; }
        if(t._from)         { renderInvite(container, t); return; }
        if(t.done)          { renderResult(container, t); return; }
        if(t.phase==='lobby') { renderLobby(container, t); return; }
        renderGame(container, t);
      }
    },
  ...
  function renderInvite(c, t) {
    ...
    w.innerHTML = `<div style="font-size:2rem;margin-bottom:8px">🃏</div><div style="font-size:.88rem;font-weight:700;margin-bottom:4px">${t._from} vous invite</div><div style="font-size:.68rem;color:var(--txt2);margin-bottom:16px">Texas Hold'em · ${t.sb}/${t.bb} · 1000🪙</div>`;
    ...
    row.appendChild(yes); row.appendChild(no); w.appendChild(row); c.appendChild(w);
  }
  ...
  function renderLobby(c, t) {
    ...
    hdr.innerHTML = `<span style="font-family:var(--mono);font-size:.8rem;color:var(--acc)">🃏 SALON · ${t.sb}/${t.bb}</span>`;
    if(t.isHost) {
      const btn = frodon.makeElement('button','plugin-action-btn acc','▶ Lancer');
      btn.style.fontSize='.65rem'; btn.addEventListener('click', () => hostDeal(t.tid)); hdr.appendChild(btn);
    }
    c.appendChild(hdr);
    ...
    t.players.forEach(p => {
      ...
      seat.style.cssText = `display:flex;flex-direction:column;align-items:center;gap:4px;padding:10px 8px;min-width:60px;background:var(--sur2);border:1.5px solid ${isMe?'var(--acc)':'var(--bdr2)'};border-radius:12px`;
      seat.innerHTML = `${avHtml(p,36)}<div style="font-size:.6rem;font-weight:700;color:${isMe?'var(--acc)':'var(--txt)'};max-width:64px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${isMe?'Moi':p.name}</div><div style="font-size:.55rem;font-family:var(--mono);color:var(--txt2)">${p.chips}🪙</div>`;
      seats.appendChild(seat);
    });
    c.appendChild(seats);
    const hint = frodon.makeElement('div',''); hint.style.cssText='text-align:center;font-size:.6rem;color:var(--txt3);padding:0 12px 14px'; hint.textContent='Invitez des pairs depuis leur fiche · 2-8 joueurs'; c.appendChild(hint);
  }

  function renderGame(c, t) {
    const myPlayer = t.players.find(p => p.id===me());
    ...
    top.innerHTML = `<div style="display:flex;gap:3px">${PHASES.map(p=>`<span style="font-size:.55rem;font-family:var(--mono);padding:2px 6px;border-radius:4px;background:${t.phase===p?'rgba(0,245,200,.15)':'transparent'};color:${t.phase===p?'var(--acc)':'var(--txt3)'}">${phaseLabel[p]}</span>`).join('')}</div><span style="font-family:var(--mono);font-size:.75rem;color:var(--warn)">Pot ${t.pot}🪙</span>`;
    c.appendChild(top);
    ...
    const hl = frodon.makeElement('div',''); hl.style.cssText='font-size:.56rem;font-family:var(--mono);color:var(--txt3);text-transform:uppercase'; hl.textContent=myPlayer?.status==='fold'?'Couché':'Mes cartes';
    ...
    if(t.myHand?.length===2) {
      ...
      if(t.community.length >= 3) {
        ...
        hw.appendChild(hl); hw.appendChild(mc); hw.appendChild(hn); c.appendChild(hw);
        renderActions(c, t, myPlayer); return;
      }
    } else {
      mc.innerHTML = cardHtml(null,true) + cardHtml(null,true);
    }
    hw.appendChild(hl); hw.appendChild(mc); c.appendChild(hw);
    renderActions(c, t, myPlayer);
  }

  function renderActions(c, t, myPlayer) {
    if(!myPlayer || myPlayer.status !== 'active' || t.players[t.currentIdx]?.id !== me()) {
      ...
      if(myPlayer?.status==='fold')  w.textContent='Couché';
      else if(myPlayer?.status==='allin') w.textContent='All-in';
      else w.textContent='⌛ Tour de '+(t.players[t.currentIdx]?.name||'?');
      c.appendChild(w); return;
    }
    const toCall = Math.min(t.roundBet - myPlayer.bet, myPlayer.chips);
    const canCheck = myPlayer.bet >= t.roundBet;
    ...
    if(canCheck) {
      const ck = frodon.makeElement('button','plugin-action-btn acc','✔ Checker'); ck.style.fontSize='.68rem'; ck.addEventListener('click', () => doAct(t,'check'));
      r1.appendChild(fold); r1.appendChild(ck);
    } else {
      const cl = frodon.makeElement('button','plugin-action-btn acc','📞 Suivre +'+toCall+'🪙'); cl.style.fontSize='.68rem'; cl.addEventListener('click', () => doAct(t,'call'));
      r1.appendChild(fold); r1.appendChild(cl);
    }
    wrap.appendChild(r1);
    const r2 = frodon.makeElement('div',''); r2.style.cssText='display:flex;gap:6px;align-items:center;justify-content:center';
    const inp = document.createElement('input'); inp.type='number'; inp.className='f-input'; inp.style.cssText='width:80px;text-align:center;padding:5px 8px;font-family:var(--mono)'; inp.min=t.roundBet+t.bb; inp.max=myPlayer.chips; inp.step=t.bb; inp.value=Math.min(t.roundBet+t.bb, myPlayer.chips);
    ...
    const allin = frodon.makeElement('button','plugin-action-btn','♠ All-in'); allin.style.cssText+=';font-size:.68rem;color:var(--warn)'; allin.addEventListener('click', () => doAct(t,'allin',myPlayer.chips));
    r2.appendChild(inp); r2.appendChild(raise); r2.appendChild(allin);
    wrap.appendChild(r2); c.appendChild(wrap);
  }
  ...
  function doAct(t, action, amount) {
    if(t.isHost) { hostAct(t.tid, me(), action, amount||0); }
      ...
      const myPlayer = t.players.find(p => p.id===me());
      if(myPlayer) {
        if(action==='fold')  myPlayer.status='fold';
        else if(action==='call')  { const x=Math.min(t.roundBet-myPlayer.bet,myPlayer.chips); myPlayer.chips-=x; myPlayer.bet+=x; }
        else if(action==='raise') { const x=Math.min((amount||t.roundBet+t.bb)-myPlayer.bet,myPlayer.chips); myPlayer.chips-=x; myPlayer.bet+=x; }
        else if(action==='allin') { myPlayer.chips=0; myPlayer.bet+=amount||0; myPlayer.status='allin'; }
  ...
  function renderResult(c, t) {
    ...
    hdr.innerHTML = `<div style="font-size:1.8rem;margin-bottom:5px">${isWin?'🏆':'🃏'}</div><div style="font-size:.95rem;font-weight:700;color:${isWin?'var(--ok)':'var(--txt)'}">${isWin?'Vous avez gagné !':(res.winnerName||'?')+' gagne'}</div><div style="font-size:.7rem;font-family:var(--mono);color:var(--warn);margin-top:3px">Pot : ${res.pot}🪙</div>`;
    c.appendChild(hdr);

    if(t.community?.length) {
      const comm = frodon.makeElement('div',''); comm.style.cssText='display:flex;gap:5px;justify-content:center;margin:0 8px 10px';
      t.community.forEach(card => { const el=frodon.makeElement('div',''); el.innerHTML=cardHtml(card); comm.appendChild(el); }); c.appendChild(comm);
    }
    ...
    if(t.isHost) {
      ...
      newH.addEventListener('click', () => hostDeal(t.tid));
      btns.appendChild(newH);
    } else {
      const w = frodon.makeElement('div',''); w.style.cssText='font-size:.64rem;color:var(--txt3);font-family:var(--mono);text-align:center;padding:6px 0'; w.textContent="⌛ L'hôte lance la prochaine main…"; btns.appendChild(w);
    }
    const leave = frodon.makeElement('button','plugin-action-btn','🚪 Quitter la table');
    leave.style.cssText += ';width:100%';
    leave.addEventListener('click', () => {
      if(t.isHost) sendAll(t.tid,'kick',{});
      else send(t.hostId,'leave',{tid:t.tid});
      delete tables[t.tid]; persist(); frodon.refreshSphereTab(PLUGIN_ID);
    });
  ...
  function renderScores(c) {
    ...
    const hist = store.get('history')||[];
    if(hist.length) {
      const l = frodon.makeElement('div','section-label','Dernières parties'); l.style.margin='0 8px 6px'; c.appendChild(l);
      hist.slice(0,10).forEach(h => {
        const isW = h.result==='win';
        ...
        row.innerHTML = `<span style="font-size:.9rem">${isW?'🏆':'😔'}</span>`;
        const inf = frodon.makeElement('div',''); inf.style.cssText='flex:1';
        inf.innerHTML = `<div style="font-size:.72rem;font-weight:700;color:${isW?'var(--ok)':'var(--warn)'}">${isW?'Victoire':'Défaite'}</div><div style="font-size:.6rem;color:var(--txt2)">vs ${(h.opponents||[]).join(', ')||'?'} · ${h.pot}🪙</div>`;
    ...
    rst.style.cssText += ';font-size:.62rem;margin:10px 8px 0;width:calc(100% - 16px);color:var(--txt3);border-color:var(--bdr)'; rst.textContent='↺ Remettre à zéro';
  ...
  /* ── Helpers ── */
  function avHtml(p, size=32) {
    const cols = ['#00f5c8','#7c4dff','#ff6b35','#00e87a','#f5c842','#ff4f8b'];
    const cl = cols[((p?.name||'?').charCodeAt(0)||0) % cols.length];
    return `<div style="width:${size}px;height:${size}px;border-radius:50%;overflow:hidden;flex-shrink:0;background:var(--sur2);display:flex;align-items:center;justify-content:center;font-size:${size*.38}px;color:${cl};font-weight:700">${(p?.name||'?')[0].toUpperCase()}</div>`;
  }
  ...
  frodon.registerUninstallHook(PLUGIN_ID, () => {
    for(const [tid,t] of Object.entries(tables)) {
      if(t.isHost) sendAll(tid,'kick',{});
      else send(t.hostId,'leave',{tid});
    }
  });
