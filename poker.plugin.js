/**
 * FRODON PLUGIN — Texas Hold'em Poker P2P  v4.2
 * Fix: fin de partie avant river, multi-table, scores, noms de profil
 */
frodon.register({
  id:'poker', name:"Texas Hold'em", version:'4.2.0',
  ...
  function hostDeal(tid) {
    ...
    if (t.players.filter(p => p.chips > 0).length < 2) {
      frodon.showToast('Pas assez de joueurs');
      return;
    }
    t.deck     = mkDeck();
    t.community = [];
    ...
    while (t.players[t.dealerIdx].chips <= 0)
      t.dealerIdx = (t.dealerIdx+1) % t.players.length;
    t.players.forEach(p => {
    ...
    const nxt = i => {
      let j = (i+1) % t.players.length;
      while (t.players[j].chips <= 0) j = (j+1) % t.players.length;
      return j;
    };
  ...
  function hostAct(tid, fromId, action, amount) {
    const t = tables[tid];
    if (!t || t.done || t.phase==='lobby') return;
    ...
    if (action === 'fold') {
      ...
      if (p.bet < t.roundBet) return;
      p.hasActed = true;
    } else if (action === 'call') {
      const x = Math.min(t.roundBet - p.bet, p.chips);
      p.chips -= x; p.bet += x;
      if (!p.chips) p.status = 'allin';
      p.hasActed = true;
    } else if (action === 'raise') {
      const r = Math.max(amount||0, t.roundBet + t.bb);
      const a = Math.min(r - p.bet, p.chips);
      p.chips -= a; p.bet += a; t.roundBet = p.bet;
      if (!p.chips) p.status = 'allin';
    ...
    while (t.players[next].status !== 'active' && guard < t.players.length) {
      next = (next+1) % t.players.length;
      guard++;
    }
    t.currentIdx = next;
    hostSync(tid);
  ...
  function hostRoundEnd(tid) {
    ...
    if (!act.length || act.every(p => p.hasActed && p.bet === t.roundBet)) {
      hostStreet(tid);
      return true;
    }
    return false;
  ...
  function hostStreet(tid) {
    ...
    while (t.players[i].status !== 'active') {
      i = (i+1) % t.players.length;
      if (i === t.dealerIdx) break;
    }
    t.currentIdx = i;
    /* FIX: si tous all-in, enchaîner automatiquement les rues */
    if (!t.players.some(p => p.status === 'active')) {
      hostSync(tid);
      setTimeout(() => hostStreet(tid), 700);
    } else {
      hostSync(tid);
    }
  }
  ...
  function hostShowdown(tid) {
    ...
    t.players.forEach(p => { t.pot += p.bet; p.bet = 0; });
    const alive = t.players.filter(p => p.status==='active' || p.status==='allin');
    const ev = alive.map(p => {
      const h = t.allHands[p.id] || [];
      const b = evalBest([...h, ...t.community]);
      return {id:p.id, name:p.name, hand:h, handName:b.name, score:b.score};
    });
  ...
  /* FIX: _scored — addScore appelé exactement une fois (race condition sync/showdown) */
  function hostEnd(tid, result) {
    ...
    t.done = true;
    t.result = result;
    result.players.forEach(r => {
      const p = t.players.find(q => q.id === r.id);
  ...
  frodon.onDM(PLUGIN_ID, (fromId, payload) => {
    ...
    if (type === 'invite') {
      for (const [k,t] of Object.entries(tables)) {
        if (t.hostId === fromId && t.done) delete tables[k];
      }
      tables[tid] = {
        tid, isHost:false, hostId:fromId,
        phase:'lobby', done:false, _scored:false,
    ...
    if (type === 'invite_ok') {
      ...
      if (!p) {
        p = {id:fromId, name:peerName(fromId), chips:1000, bet:0, hasActed:false, status:'active'};
        t.players.push(p);
      } else {
        p.status = 'active';
      }
    ...
    if (type === 'invite_no') {
      ...
      frodon.showToast(peerName(fromId)+' décline.');
      hostSync(tid);
      return;
    }
    ...
    if (type === 'action') {
      const t = tables[tid];
      if (!t || !t.isHost) return;
      hostAct(tid, fromId, payload.action, payload.amount||0);
      return;
    }
    ...
    if (type === 'resync') {
      ...
      let p = t.players.find(pl => pl.id === fromId);
      if (!p) {
        const nm = peerName(fromId);
        p = t.players.find(pl => pl.status==='away' && pl.name === nm);
        if (p) {
          if (t.allHands[p.id]) { t.allHands[fromId] = t.allHands[p.id]; delete t.allHands[p.id]; }
          p.id = fromId;
        }
      }
  ...
  frodon.onPeerAppear(peer => {
    for (const [tid,t] of Object.entries(tables)) {
      const p = t.players.find(pl => pl.id===peer.peerId) ||
                t.players.find(pl => pl.status==='away' && pl.name===peer.name);
      if (!p) continue;
      frodon.showToast('🃏 '+peer.name+' est de retour !');
      if (t.isHost) {
        if (p.id !== peer.peerId) {
          if (t.allHands[p.id]) { t.allHands[peer.peerId]=t.allHands[p.id]; delete t.allHands[p.id]; }
          p.id = peer.peerId;
        }
  ...
  frodon.registerPeerAction(PLUGIN_ID, '🃏 Poker', (peerId, container) => {
    ...
    /* FIX: ne compte que les parties en cours (non terminées) */
    const t = Object.values(tables).find(g => !g.done && g.players.some(p => p.id===peerId));
    if (!t) {
  ...
  /* ── SPHERE ── FIX: affiche toutes les tables */
  ...
  function W(c) {
    ...
    w.style.cssText = 'border-bottom:1px solid var(--bdr);margin-bottom:2px';
    c.appendChild(w);
    return w;
  }
  function avH(p, sz=32) {
    ...
    const nm = p?.name || '?';
    const cl = cols[nm.charCodeAt(0) % cols.length];
  ...
  function renderInvite(c, t) {
    ...
    yes.addEventListener('click', () => {
      send(t.hostId, 'invite_ok', {tid:t.tid});
      delete t._from;
      persist();
      frodon.showToast('🃏 Rejoint !');
      frodon.refreshSphereTab(PLUGIN_ID);
    });
    const no = frodon.makeElement('button','plugin-action-btn','✕ Refuser');
    no.addEventListener('click', () => {
      send(t.hostId, 'invite_no', {tid:t.tid});
      delete tables[t.tid];
      persist();
      frodon.refreshSphereTab(PLUGIN_ID);
    });
  ...
  function renderLobby(c, t) {
    ...
    if (t.isHost) {
      ...
      btn.addEventListener('click', () => hostDeal(t.tid));
      hdr.appendChild(btn);
    }
    w.appendChild(hdr);
    ...
    t.players.forEach(p => {
      ...
      seat.innerHTML = `${avH(p,36)}<div style="font-size:.6rem;font-weight:700;color:${isMe?'var(--acc)':'var(--txt)'};max-width:64px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${isMe?'Moi':p.name}</div><div style="font-size:.55rem;font-family:var(--mono);color:var(--txt2)">${p.chips}🪙</div>`;
      seats.appendChild(seat);
    });
    w.appendChild(seats);
    const hint = frodon.makeElement('div','');
    hint.style.cssText = 'text-align:center;font-size:.6rem;color:var(--txt3);padding:0 12px 14px';
    hint.textContent = 'Invitez des pairs depuis leur fiche · 2-8 joueurs';
    w.appendChild(hint);
  }
  ...
  function renderGame(c, t) {
    ...
    w.appendChild(top);
    const prow = frodon.makeElement('div','');
    prow.style.cssText = 'display:flex;flex-wrap:wrap;gap:4px;justify-content:center;padding:8px 6px 4px';
    t.players.forEach((p, i) => {
      const isMe  = p.id === me();
      const isCur = i === t.currentIdx;
      const isD   = i === t.dealerIdx;
    ...
    w.appendChild(comm);
    const hw = frodon.makeElement('div','');
    hw.style.cssText = 'display:flex;flex-direction:column;align-items:center;gap:5px;padding:6px 8px';
    const hl = frodon.makeElement('div','');
    hl.style.cssText = 'font-size:.56rem;font-family:var(--mono);color:var(--txt3);text-transform:uppercase';
    hl.textContent = myP?.status==='fold' ? 'Couché' : 'Mes cartes';
    const mc = frodon.makeElement('div','');
    mc.style.cssText = 'display:flex;gap:6px';
    if (t.myHand?.length === 2) {
      ...
      if (t.community.length >= 3) {
        ...
        hw.appendChild(hl); hw.appendChild(mc); hw.appendChild(hn);
        w.appendChild(hw);
        renderActions(w, t, myP);
        return;
      }
    } else {
      mc.innerHTML = cardHtml(null,true) + cardHtml(null,true);
    }
    hw.appendChild(hl); hw.appendChild(mc);
    w.appendChild(hw);
    renderActions(w, t, myP);
  }
  ...
  function renderActions(c, t, myP) {
    ...
    const r1 = frodon.makeElement('div','');
    r1.style.cssText = 'display:flex;gap:6px;flex-wrap:wrap;justify-content:center;margin-bottom:7px';
    const fold = frodon.makeElement('button','plugin-action-btn','🏳 Coucher');
    fold.style.fontSize = '.68rem';
    fold.addEventListener('click', () => doAct(t,'fold'));
    if (canCheck) {
      const ck = frodon.makeElement('button','plugin-action-btn acc','✔ Checker');
      ck.style.fontSize = '.68rem';
      ck.addEventListener('click', () => doAct(t,'check'));
      r1.appendChild(fold); r1.appendChild(ck);
    } else {
      const cl = frodon.makeElement('button','plugin-action-btn acc','📞 Suivre +'+toCall+'🪙');
      cl.style.fontSize = '.68rem';
      cl.addEventListener('click', () => doAct(t,'call'));
      r1.appendChild(fold); r1.appendChild(cl);
    }
    wrap.appendChild(r1);
    const r2 = frodon.makeElement('div','');
    r2.style.cssText = 'display:flex;gap:6px;align-items:center;justify-content:center';
    ...
    inp.min   = t.roundBet + t.bb;
    inp.max   = myP.chips;
    inp.step  = t.bb;
    inp.value = Math.min(t.roundBet + t.bb, myP.chips);
    const raise = frodon.makeElement('button','plugin-action-btn','🔺 Relancer');
    raise.style.fontSize = '.68rem';
    raise.addEventListener('click', () => doAct(t,'raise',+inp.value||t.roundBet+t.bb));
    const allin = frodon.makeElement('button','plugin-action-btn','♠ All-in');
    allin.style.fontSize = '.68rem';
    allin.style.color    = 'var(--warn)';
    allin.addEventListener('click', () => doAct(t,'allin',myP.chips));
  ...
  function doAct(t, action, amount) {
    if (t.isHost) {
      hostAct(t.tid, me(), action, amount||0);
    } else {
  ...
  function renderResult(c, t) {
    ...
    lbl.style.cssText = 'font-size:.56rem;color:var(--txt3);font-family:var(--mono);text-transform:uppercase;letter-spacing:.8px;margin-bottom:5px';
    lbl.textContent = 'Chips restants';
    chips.appendChild(lbl);
    t.players.forEach(p => {
      const isMe = p.id === me();
      const row  = frodon.makeElement('div','');
      row.style.cssText = 'display:flex;justify-content:space-between;font-size:.68rem;font-family:var(--mono);padding:2px 0';
      const nm = frodon.makeElement('span','', isMe?'Moi':p.name);
      nm.style.color = isMe ? 'var(--acc)' : 'var(--txt2)';
      const ch = frodon.makeElement('span','', p.chips>0 ? p.chips+'🪙' : '💀 0');
      ch.style.color = p.chips > 0 ? 'var(--txt2)' : 'var(--warn)';
      row.appendChild(nm); row.appendChild(ch);
      chips.appendChild(row);
    });
    w.appendChild(chips);
    const btns = frodon.makeElement('div','');
    btns.style.cssText = 'padding:12px 8px 14px;display:flex;flex-direction:column;gap:8px';
    if (t.isHost) {
      ...
      msg.style.cssText = 'font-size:.64rem;color:var(--txt3);font-family:var(--mono);text-align:center;padding:6px 0';
      msg.textContent = "⌛ L'hôte lance la prochaine main…";
  ...
  function renderScores(c) {
    ...
    if (!total) {
      c.innerHTML = '<div style="text-align:center;padding:28px 14px;color:var(--txt2);font-size:.72rem"><div style="font-size:1.6rem;opacity:.2;margin-bottom:6px">🏆</div>Jouez votre première partie !</div>';
      return;
    }
    ...
    [['🏆',wins,'Victoires','var(--ok)'],['😔',losses,'Défaites','var(--warn)']].forEach(([ico,n,lbl,col],i) => {
      ...
      cell.style.cssText = 'flex:1;display:flex;flex-direction:column;align-items:center;gap:1px;padding:14px 4px;'+(i===0?'border-right:1px solid var(--bdr2)':'');
    ...
    const bw = frodon.makeElement('div','');
    bw.style.cssText = 'margin:0 8px 10px';
    ...
    c.appendChild(bw);
    if (chipsWon > 0) {
      const cw = frodon.makeElement('div','');
      cw.style.cssText = 'margin:0 8px 10px;padding:8px 10px;background:rgba(0,229,122,.07);border:1px solid rgba(0,229,122,.2);border-radius:8px;display:flex;justify-content:space-between';
      cw.innerHTML = `<span style="font-size:.65rem;color:var(--txt2)">Chips gagnés</span><strong style="font-family:var(--mono);color:var(--ok)">${chipsWon.toLocaleString()}🪙</strong>`;
      c.appendChild(cw);
    }
    const hist = store.get('history') || [];
    if (hist.length) {
      const l = frodon.makeElement('div','section-label','Dernières parties');
      l.style.margin = '0 8px 6px';
      c.appendChild(l);
      hist.slice(0,10).forEach(h => {
        const isW = h.result === 'win';
        const row = frodon.makeElement('div','');
        row.style.cssText = 'display:flex;align-items:center;gap:8px;padding:5px 10px;border-bottom:1px solid var(--bdr)';
        row.innerHTML = `<span style="font-size:.9rem">${isW?'🏆':'😔'}</span>`;
        const inf = frodon.makeElement('div','');
        inf.style.cssText = 'flex:1';
        inf.innerHTML = `<div style="font-size:.72rem;font-weight:700;color:${isW?'var(--ok)':'var(--warn)'}">${isW?'Victoire':'Défaite'}</div><div style="font-size:.6rem;color:var(--txt2)">vs ${(h.opponents||[]).join(', ')||'Pair inconnu'} · ${h.pot}🪙</div>`;
        row.appendChild(inf);
        row.appendChild(frodon.makeElement('span','mini-card-ts',frodon.formatTime(h.ts)));
        c.appendChild(row);
      });
    }
    ...
    rst.addEventListener('click', () => {
      ...
      ['wins','losses','chips_won','history'].forEach(k => store.del(k));
      frodon.refreshSphereTab(PLUGIN_ID);
    });
    c.appendChild(rst);
  }
