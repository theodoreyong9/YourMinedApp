...
(function () {
  ...
  const GLOBAL_UPGRADES = [
    ...
    {id:'interest',  name:'Banque',      emoji:'🏦', cost:400,desc:'3% intérêts sur l\'or/vague',   effect:{interest:0.03}},
    {id:'detect',    name:'Radar',       emoji:'📡', cost:200,desc:'Toutes tours détectent furtif',  effect:{detectStealth:true}},
  ...
  function _storePendingInvite(inv){localStorage.setItem('ym_td_pending_invite',JSON.stringify(inv));}
  ...
  function renderSoloGame(container){
      ...
      ws.squads.forEach(sq=>{
        ...
        for(let i=0;i<sq.count;i++){
          sc.time.delayedCall(offset+i*sq.delay,()=>{
            ...
            spawnEnemy(sc,{hp:Math.round(def.hp*scale),spd:Math.min(def.spd+(waveIdx-1)*1.5,160),reward:Math.round(def.rew*scale),col:def.col,shape:def.shape,size:def.size,armor:def.armor||0,flying:!!def.flying,stealth:!!def.stealth,heals:!!def.heals,splits:!!def.splits,rages:!!def.rages,boss:!!def.boss,name:def.name,typeid:sq.type});
          });
        }
      });
    ...
    function showWaveShop(sc){
      ...
      const offers=[...available].sort(()=>Math.random()-.5).slice(0,3);
      const ov=document.createElement('div');
      ov.id='td-shop-ov';
      ...
      let offerHtml=offers.map(u=>{
        ...
        return `<div style="padding:8px;border-radius:8px;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);margin-bottom:7px">
          ...
          <button data-id="${u.id}" class="shop-btn" style="width:100%;padding:6px;background:${can?'rgba(16,185,129,.15)':'rgba(100,100,100,.1)'};border:1px solid ${can?'#10b981':'#444'};color:${can?'#10b981':'#555'};border-radius:6px;cursor:${can?'pointer':'default'};font-size:10px;font-family:monospace">${can?`✓ Acheter (${u.cost}g)`:`✗ Besoin de ${u.cost}g`}</button>
        </div>`;
      }).join('');
      ...
      ov.innerHTML=`<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">
        ...
        <div style="font-size:10px;color:rgba(255,255,255,.3);margin-bottom:10px">💰 Trésorerie: ${gold.toLocaleString()}g</div>
        ${offerHtml}
      ...
      ov.querySelectorAll('.shop-btn').forEach(btn=>{
        btn.addEventListener('click',()=>{
          const u=GLOBAL_UPGRADES.find(x=>x.id===btn.dataset.id);
          if(!u||gold<u.cost||purchasedUpgrades.has(u.id))return;
          gold-=u.cost; purchasedUpgrades.add(u.id); applyGlobalUpg(u.effect); updateHUD();
