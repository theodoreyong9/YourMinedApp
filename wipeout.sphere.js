...
// wipeout.sphere.js — Anti-Gravity Racer v10
// Fix écran noir : le canvas est créé avec renderer.domElement (Three.js le gère),
// les dimensions sont obtenues sur le container racine du panel via une chaîne de
// fallbacks robuste. Architecture sans sous-containers intermédiaires.
(function () {
  'use strict';
  window.YM_S = window.YM_S || {};

  const SCORES_KEY = 'ym_wipeout_scores_v10';
  ...
  // ── PANEL ──────────────────────────────────────────────────────────────────
  // rootContainer = le div que le système Sphere nous passe (le vrai container)
  // On y met TOUT directement, pas de sous-div flex
  let _rootContainer = null;

  function renderPanel(container) {
    _rootContainer = container;
    container.style.cssText='position:relative;width:100%;height:100%;overflow:hidden;background:#000;font-family:monospace;display:flex;flex-direction:column';
    container.innerHTML='';
    showMenu(container);
  }
  ...
  function showMenu(container) {
    ...
    [['menu','🏁 Course'],['scores','🏆 Temps']].forEach(([id,lbl],i)=>{
      ...
      t.onclick=()=>{
        ...
        if(id==='menu') buildMenuContent(scroll, container);
        else buildScoresContent(scroll);
      };
      tabs.appendChild(t);
    });
    ...
    buildMenuContent(scroll, container);
  }

  function buildScoresContent(scroll) {
    const sc=loadScores().sort((a,b)=>a.time-b.time);
    let h=`<div style="padding:14px"><div style="font-size:15px;font-weight:700;color:#00ccff;margin-bottom:12px">🏆 Meilleurs temps</div>`;
  ...
  function buildMenuContent(scroll, rootContainer) {
    ...
    function build() {
      ...
      scroll.innerHTML=`<div style="display:flex;flex-direction:column;align-items:center;padding:16px 12px;background:linear-gradient(180deg,#000,#020218);min-height:100%">
        ...
        <div style="display:flex;flex-direction:column;gap:6px;width:100%;margin-bottom:16px">
          ${Object.entries(SHIPS).map(([id,s])=>{
            const sel=id===selShip, colHex=s.col.toString(16).padStart(6,'0');
            return `<div data-ship="${id}" style="padding:10px 12px;border-radius:8px;border:1.5px solid ${sel?`#${colHex}`:'rgba(255,255,255,.08)'};cursor:pointer;background:${sel?'rgba(255,255,255,.04)':'transparent'}">
              ...
              ${sel?`<div style="display:flex;flex-direction:column;gap:4px">
                <div style="display:flex;align-items:center;gap:8px"><span style="font-size:8px;color:rgba(255,255,255,.35);width:50px">ACCEL</span>${bar(s.accel,28,s.col)}</div>
                <div style="display:flex;align-items:center;gap:8px"><span style="font-size:8px;color:rgba(255,255,255,.35);width:50px">VITESSE</span>${bar(s.maxSpd,38,s.col)}</div>
                <div style="display:flex;align-items:center;gap:8px"><span style="font-size:8px;color:rgba(255,255,255,.35);width:50px">GRIP</span>${bar(s.grip*100,100,s.col)}</div>
                <div style="display:flex;align-items:center;gap:8px"><span style="font-size:8px;color:rgba(255,255,255,.35);width:50px">BOUCLIER</span>${bar(s.shield,100,s.col)}</div>
                <div style="display:flex;align-items:center;gap:8px"><span style="font-size:8px;color:rgba(255,255,255,.35);width:50px">BOOST</span>${bar(s.boostMult,3,s.col)}</div>
              </div>`:''}
            </div>`;
          }).join('')}
        </div>
      ...
      scroll.querySelectorAll('[data-track]').forEach(el=>el.onclick=e=>{e.stopPropagation();selTrack=el.dataset.track;build();});
      scroll.querySelectorAll('[data-ship]').forEach(el=>el.onclick=e=>{e.stopPropagation();selShip=el.dataset.ship;build();});

      scroll.querySelector('#wo-go').onclick=()=>{
        // Taille du container racine — utiliser TOUTES les méthodes disponibles
        const {W,H} = getContainerSize(rootContainer);
        startRace(rootContainer, selTrack, selShip, W, H);
      };
    }
    build();
  }
  ...
  // ── TEXTURE PISTE ──────────────────────────────────────────────────────────
  function buildTrackTexture(THREE, T){
    ...
    for(let y=0;y<TS;y++) for(let x=0;x<TS;x++){
      ...
      if((Math.abs(u-.33)<.009||Math.abs(u-.67)<.009)&&(v*60%1)<.5){r=Math.max(r,Math.round(cr*.25));g=Math.max(g,Math.round(cg*.25));b=Math.max(b,Math.round(cb*.25));}
      ...
      TD[i4]=Math.min(255,r);TD[i4+1]=Math.min(255,g);TD[i4+2]=Math.min(255,b);TD[i4+3]=255;
    }
  ...
  // ── COURSE ─────────────────────────────────────────────────────────────────
  function startRace(rootContainer, trackId, shipId, W, H) {
    if(!window.THREE){
      const s=document.createElement('script');
      s.src='https://cdnjs.cloudflare.com/ajax/libs/three.js/r128/three.min.js';
      s.onload=()=>startRace(rootContainer,trackId,shipId,W,H);
    ...
    // Renderer Three.js — on laisse Three créer son propre canvas
    ...
    const ro=new ResizeObserver(()=>{
      const nw=rootContainer.clientWidth||W, nh=rootContainer.clientHeight||H;
      if(!nw||!nh) return;
    ...
    rootContainer.appendChild(speedo);
    ...
    cntDiv.appendChild(cntTxt); rootContainer.appendChild(cntDiv);
    ...
    rootContainer.appendChild(dmgFlash);
    ...
    bstBtn.id='wbs';
    bstBtn.style.cssText='width:84px;height:84px;background:rgba(100,0,255,.18);border:2px solid rgba(100,0,255,.5);color:#bb99ff;font-size:26px;border-radius:50%;cursor:pointer;font-family:monospace;display:flex;align-items:center;justify-content:center;flex-direction:column;touch-action:none';
    bstBtn.innerHTML='⚡<span style="font-size:9px;letter-spacing:1px">BOOST</span>';
    ctrlDiv.appendChild(bstBtn);
    rootContainer.appendChild(ctrlDiv);
    ...
    rootContainer.appendChild(finDiv);
    ...
    const gnd=new THREE.Mesh(new THREE.PlaneGeometry(2000,2000,20,20),new THREE.MeshLambertMaterial({color:gndCol}));
    ...
    // Piste
    const curve=new THREE.CatmullRomCurve3(T.pts(),true,'catmullrom',.5);
    const N=SEG, frames=curve.computeFrenetFrames(N,true), TW=T.width;

    function trackRight(fi){const tang=frames.tangents[fi%N];const r=new THREE.Vector3(-tang.z,0,tang.x).normalize();return r.lengthSq()<.001?new THREE.Vector3(1,0,0):r;}

    const sPos=[],sUV=[],sIdx=[];
    for(let i=0;i<=N;i++){
      const pt=new THREE.Vector3(); curve.getPointAt(i/N,pt); const r=trackRight(i);
    ...
    scene.add(new THREE.Mesh(sGeo,new THREE.MeshStandardMaterial({map:buildTrackTexture(THREE,T),roughness:.12,metalness:.95,side:THREE.DoubleSide})));

    function buildRail(side){
      const lo=[],hi=[];
      for(let i=0;i<=N;i++){const pt=new THREE.Vector3();curve.getPointAt(i/N,pt);const r=trackRight(i);lo.push(new THREE.Vector3(pt.x+r.x*side*(TW/2+.3),pt.y+.25,pt.z+r.z*side*(TW/2+.3)));hi.push(new THREE.Vector3(pt.x+r.x*side*(TW/2+.3),pt.y+3.2,pt.z+r.z*side*(TW/2+.3)));}
    ...
    for(let i=0;i<BLDG;i++){
      const t=i/BLDG,pt=new THREE.Vector3();curve.getPointAt(t,pt);
      ...
      if(rng.n()<.18){const pl=new THREE.PointLight(wc,1.5,22);pl.position.set(bx,pt.y+bh*.6,bz);scene.add(pl);}
    }
    ...
    for(let i=0;i<N;i+=PSTEP){
      ...
      [-1,1].forEach(s=>{
        ...
        const pl=new THREE.PointLight(s<0?T.col:T.col2,4,14);pl.position.copy(ball.position);scene.add(pl);
      });
    }

    const pads=[];
    for(let i=0;i<8;i++){
      const t=(i+.5)/8,pt=new THREE.Vector3();curve.getPointAt(t,pt);
    ...
    for(let i=0;i<GATES;i++){
      const t=(i+.5)/GATES,pt=new THREE.Vector3();curve.getPointAt(t,pt);
    ...
    [-1,1].forEach(s=>{
      ...
      const wg=new THREE.BufferGeometry();wg.setAttribute('position',new THREE.BufferAttribute(wv,3));wg.setIndex([0,1,2,2,3,0,0,2,1,0,3,2]);wg.computeVertexNormals();ship.add(new THREE.Mesh(wg,darkM));
      const pv=new Float32Array([0,.09,2.0,s*3.2,-.02,-.2,s*2.2,-.01,-1.8,0,.08,-1.8]);
      const pg=new THREE.BufferGeometry();pg.setAttribute('position',new THREE.BufferAttribute(pv,3));pg.setIndex([0,1,2,2,3,0]);pg.computeVertexNormals();ship.add(new THREE.Mesh(pg,accentM));
    ...
    [-0.7,.7].forEach(ex=>{
      ...
      const gl=new THREE.Mesh(glG,new THREE.MeshBasicMaterial({color:0x00ccff,transparent:true,opacity:.9}));
      gl.position.set(ex,-.12,-3.9);ship.add(gl);engineGlows.push(gl);
    ...
    let vel=new THREE.Vector3(),heading=Math.atan2(startTan.x,startTan.z),vertVel=0;
    let speed=0,roll=0,pitch=0,hoverPhase=0,boost=.8,shield=SD.shield;
    ...
    const bindBtn=(id,key)=>{
      const el=rootContainer.querySelector('#'+id);if(!el)return;
    ...
    function nearestT(p){let best=_lastT,bestD=1e18;const W2=.18,steps=isMobile?40:60;for(let i=0;i<steps;i++){const t=((_lastT-W2+i*(W2*2/steps))%1+1)%1;curve.getPointAt(t,_tv);const d=_tv.distanceToSquared(p);if(d<bestD){bestD=d;best=t;}}_lastT=best;return best;}
    ...
    function loop(ts){
      ...
      if(!done){
        ...
        if(cdDone){
          ...
          if(keys.l)heading+=turn*dt; if(keys.r)heading-=turn*dt;
          ...
          if(started&&_lastT>.85&&ct<.15&&cpPassed.size>=GATES){
            cpPassed.clear();const lt=Date.now()-lapStart;lapTimes.push(lt);lapStart=Date.now();laps++;
          ...
          const m4=new THREE.Matrix4();m4.lookAt(new THREE.Vector3(0,0,0),fwd.clone().negate(),new THREE.Vector3(0,1,0));ship.quaternion.setFromRotationMatrix(m4);
          const turnIn=(keys.r?1:0)-(keys.l?1:0);
          roll+=((-turnIn*.55)-roll)*dt*7;pitch+=((keys.u?.06:keys.d?-.04:0)-pitch)*dt*6;
    ...
    function crash(){
      ...
      setTimeout(()=>{
        ...
        finDiv.querySelector('#fin-r').onclick=()=>{stopRace();startRace(rootContainer,trackId,shipId,W,H);};
        finDiv.querySelector('#fin-m').onclick=()=>{stopRace();showMenu(rootContainer);};
      },900);
    }
  ...
  window.YM_S['wipeout.sphere.js']={
    ...
    description:'WipeOut v10 — renderer.domElement (Three gère le canvas), taille via chaîne parentale robuste',
    emit:[], receive:[],
