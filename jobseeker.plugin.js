/**
 * FRODON PLUGIN — Cherche un emploi  v1.0.0
 * Affichez un badge visible sur votre profil et dans le radar.
 * Précisez le poste recherché, les compétences, la localisation voulue.
 * Les pairs voient votre badge et peuvent vous envoyer des opportunités.
 */
frodon.register({
  id: 'jobseeker',
  ...
  frodon.registerPeerAction(PLUGIN_ID, '💼 Emploi', (peerId, container) => {
    ...
    sendBtn.addEventListener('click', () => {
      ...
      frodon.sendDM(peerId, PLUGIN_ID, { type: 'opportunity', message: msg });
  ...
  frodon.registerBottomPanel(PLUGIN_ID, [
    {
      ...
      render(container) {
        ...
        toggle_lbl.style.cssText = 'font-size:.78rem;font-weight:700;color:var(--txt)';
        ...
        const fields = [
          { key: 'jobTitle',  label: 'Poste recherché *', placeholder: 'ex: Développeur Front-end, Designer UX…', required: true },
          { key: 'skills',    label: 'Compétences clés',   placeholder: 'ex: React, Figma, Python…' },
          { key: 'location',  label: 'Localisation souhaitée', placeholder: 'ex: Paris, Remote, IDF…' },
